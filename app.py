...
# Primary model: Gemini 3 Flash Preview (V1Beta)
GEMINI_3_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash-preview:generateContent"
# Fallback model: Gemini 2.5 Flash (Stable V1Beta)
GEMINI_2_5_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent"
...
def get_gemini_suggestion(prompt_text):
    ...
    # Merging system instructions into the prompt for maximum compatibility
    full_prompt = (
        "You are a health assistant providing general, non-medical advice. "
        "Offer actionable, positive, and simple suggestions for a healthier lifestyle based on the user's information. "
        "Do not diagnose or recommend specific treatments. Always advise consulting a healthcare professional for personalized advice.\n\n"
        f"User Data: {prompt_text}"
    )
    ...
    try:
        print(f"--- Attempting Gemini 3 Flash (V1) ---")
        response = requests.post(f"{GEMINI_3_URL}?key={GEMINI_API_KEY}", headers=headers, data=json.dumps(payload))
        
        if response.status_code == 200:
            result = response.json()
            return result.get('candidates', [])[0].get('content', {}).get('parts', [])[0].get('text', 'No suggestion generated.')

        print(f"Gemini 3 Flash failed (Status: {response.status_code}). Error: {response.text}")
        
        # Fallback to Gemini 2.5 Flash
        print(f"--- Attempting Fallback: Gemini 2.5 Flash (V1Beta) ---")
        response = requests.post(f"{GEMINI_2_5_URL}?key={GEMINI_API_KEY}", headers=headers, data=json.dumps(payload))
        
        if response.status_code == 200:
            result = response.json()
            return result.get('candidates', [])[0].get('content', {}).get('parts', [])[0].get('text', 'No suggestion generated.')
        
        print(f"Gemini 2.5 Flash also failed (Status: {response.status_code}). Error: {response.text}")
        return f"API Error ({response.status_code}). Please check your API key."

    except Exception as e:
        print(f"Critical Error in get_gemini_suggestion: {e}")
